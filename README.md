...
`uv sync` for development); the plugin route installs the MCP tools and two hooks
(`SubagentStop`, `PostToolUse`) that run a project's gate around every subagent
dispatch — armed only where the project has opted in with `.convoy/gate.toml` and
this machine has trusted it with `convoy gate --trust`.
...
| `convoy unlock <series.toml>` | Release a stale run lock and nothing else — the safe half of `clean`. No discarded changes, no deleted branches; what `status` recommends for a `dead` run you mean to `--resume`, since `--resume` needs the branches `clean` would delete. Refuses when the lock's owner is still alive — this is `stale`, not `clean` | `--workspace <dir>` / `-w`, `--force` (release it anyway) |
| `convoy hook` | Run the project gate as a Claude Code hook around subagent dispatch. Reads the hook event on stdin. On `SubagentStop` (the judge) a blocking red exits 2 with the repair brief on stderr, which the subagent receives as the reason it may not stop yet — one repair round, then it may stop; read-only subagents are not gated. On `PostToolUse` for `Agent`/`Task` (the messenger, synchronous dispatch) the judge's verdict is reused and a residual red exits 2 with the brief shown to the orchestrator, its cue to dispatch a fix subagent. Silent (exit 0) when no `.convoy/gate.toml` is found or the gate is green; exit 2 with a one-line reason when the gate cannot run. A `[convoy-phase: <tag>]` marker in the subagent's brief scopes the gate. One JSON line per firing goes to `.convoy/hook.log`. Runs a project's checks only where this machine trusts the project and its spec is the one that was trusted (`convoy gate --trust` pins the hash); an untrusted spec executes nothing and leaves no file, a changed one is refused loudly. Orchestrator feedback needs a synchronous dispatch (`run_in_background: false`); the default asynchronous dispatch is judged at the subagent's stop only. Exit codes are the hook protocol's, not convoy's | (the plugin's `hooks/hooks.json` wires it; a project can wire the same command in its own `.claude/settings.json`) |
...
hooks included, so they never fire inside a governed run; a hook a project wires in
its own `.claude/settings.json` survives isolation, which is why the hook refuses a
workspace a run holds the lock on (the workspace's
...
assembly, no consumer hook API inside a run (the plugin's hooks are the gate on the
orchestrator's side, outside any run), and telemetry is economy + gate
outcomes — not
