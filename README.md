...
| `convoy unlock <series.toml>` | Release a stale run lock and nothing else — the safe half of `clean`. No discarded changes, no deleted branches; what `status` recommends for a `dead` run you mean to `--resume`, since `--resume` needs the branches `clean` would delete | `--workspace <dir>` / `-w` |
