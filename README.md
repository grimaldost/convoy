...
|------|------|---------------|
| `convoy validate <series.toml>` | Free preflight (no git mutation, no spawn). A gate-only file — just `[series] id` + `[[checks]]` — is validated as the gate it is: the isolation of every blocking independent check is probed, and stdout says `ok (gate-only)` | `--workspace <dir>` / `-w` (default: cwd) |
