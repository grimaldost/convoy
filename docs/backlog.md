...
biggest per-halt $ recovery) → T19a (per-phase checks — the only row that
unblocks a *capability* rather than recovering cost) → T13a (one-line env strip
protecting the fix loop) → T20a → T12b → T14b → T15a → T4a.**
...
| T19a | Per-phase `[[checks]]` scoping: a check declares the phases it gates (`phases = ["core"]`), defaulting to `[]` = gates every PR, so existing series are bit-for-bit unaffected. The join key is already public — `[[prs]].phase` is parsed and serialized but read by nothing today (`spec.py:354,439` are its only uses in `src/`). Shape: the field on `Check`, a pure selector in `core/gate.py`, the filter applied at both `gate_runner.run(workspace, series.checks)` sites, and a preflight problem for a `phases` entry no PR declares. Without it an *incremental* series is unrunnable — PR1's gate includes PR4's tests — which forces every real series into one fat PR or per-PR full-suite green, leaving `depends_on` (the headline feature) with no workable incremental use case. Revises the "the gate is series-global" note in `docs/design/02-formats.md`. **(consumer-affecting)** | `core/spec.py`, `core/gate.py`, `core/preflight.py`, `interface/drivers/headless.py:292,332` | proposed |
