...
T15a, T4a, T14c (shipped 0.6.0), T24a, T21a.** The stated order is **exhausted** —
every row it named has been built, and so has everything that was `proposed` when it
ran out.

What is left was surfaced by building, not by triage, and has no agreed sequence:
**T25a** (advisories never reach the run path) is the one carrying an argument, and
it gates the rest — T23a's payoff routes through that channel, and T26a's would too.
T23a itself moved to `watch` on measured evidence rather than being built; its row
carries what the measurement found and what would move it back.
