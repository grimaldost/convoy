...
to use the tools, and the full series.toml schema. `hooks/hooks.json` ships the
`PostToolUse` hook on `Agent|Task` (`uv run --project ${CLAUDE_PLUGIN_ROOT} convoy
hook`, timeout 1800 s), auto-registered when the plugin is enabled; it is inert until
...
| `convoy hook` | — | CLI-only: the gate as a `PostToolUse` hook on subagent dispatch (`interface/hook.py`). Reads the event JSON on stdin, discovers the project spec from `$CLAUDE_PROJECT_DIR` then the event's `cwd`, runs `gate_service.run_gate`, and answers in the hook protocol's vocabulary: exit 0 silent (no spec, non-dispatch tool, incomplete dispatch, or green), exit 2 with the repair brief (blocking red) or a one-line reason (the gate could not run) on stderr. `[convoy-phase: <tag>]` in `tool_input.prompt` selects phases. One JSON line per firing in `.convoy/hook.log`. Executes a project's checks only where the project root is on the per-machine trust list (`gate_service.is_trusted`, `CONVOY_HOME/hook-trust.toml`, written by `convoy gate --init` / `--trust`); an untrusted spec is logged and not run. No MCP twin: the caller is Claude Code itself |
