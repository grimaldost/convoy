...
| `convoy validate SERIES [--workspace DIR]` | `convoy_run(..., dry_run=true)` | same pre-flight for a full series; neither spawns (seat probe included) nor mutates. Advisories print to stderr / fill the `advisories` key, and change neither the exit code nor `ok`/`outcome` — so `validate` can write to stderr and still exit `0`. **One deliberate asymmetry:** the CLI additionally accepts a gate-only file (no `[branches]`/`[paths]`/`[review]`/`[[prs]]`), validating it against the gate's spec-decidable refusals and printing `ok (gate-only)`; the MCP dry-run loads with `load_series` only and answers `outcome: "usage"`. The tool's caller already has `convoy_gate` for that file, and its envelope says more than a dry-run's `ok` would |
| `convoy gate [SERIES] [--workspace DIR] [--phase TAG]... [--json] [--brief]` | `convoy_gate(workspace, series_file?, phases, brief)` | the gate standalone — run the series' `[[checks]]` once against the workspace, through the same runner and verdict as a run's per-PR gate; no spawn, no branch, no lock, no telemetry. `SERIES` may be a full series.toml or a minimal `[series] id` + `[[checks]]` file (`load_gate_spec`); omitted on either surface, the project spec is discovered from the workspace — `$CONVOY_GATE_SPEC` when the launching process set it (refused when missing), then `$CLAUDE_PROJECT_DIR/.convoy/gate.toml`, then `.convoy/gate.toml` in the workspace and its parents (`gate_service.find_gate_spec`); a spec found outside `.convoy/` governs the workspace (`gate_root`): trust, the hook log and the oracles default key on the workspace, not on the directory the file sits in; a discovered spec must be trusted on this machine (the model chose the workspace argument, so the tool holds it to the hook's standard), an explicit `series_file` is the caller's act — and a project spec loads with `CONVOY_ORACLES` defaulted to `~/.convoy/oracles/<project dir name>` (`gate_spec_env`). Four refusals map to usage rather than a verdict: an unknown phase tag, a selection with no blocking check, an empty selection, unbacked isolation on a blocking independent check (the run reports the same defect at pre-flight); no spec found is a fifth, `error_kind: spec`. Both surfaces emit the same envelopes from `gate_envelope` / `gate_usage_envelope` — the CLI's `--json` usage paths included. The envelope carries `repair_brief` (`core.gate.repair_brief`, the same failing-checks section the run's fix loop briefs a fix spawn with, `''` when green) and `convoy_version`, so a caller orchestrating its own repair appends the section rather than rebuilding it. `--brief` / `brief=true` return `gate_brief_envelope` — `{ok, outcome, repair_brief, convoy_version}` only — for a caller reading the verdict inside a model turn. **CLI-only:** `--init [--independent NAME]` scaffolds the project spec (`interface/gate_scaffold.py`) — a scaffold is an operator act, and `convoy_init` scaffolds the runner's starter series, not a project gate |
| `convoy hook` | — | CLI-only: the gate as a Claude Code hook around subagent dispatch (`interface/hook.py`), two legs. `SubagentStop` is the judge: it reads the subagent transcript (`agent_transcript_path`) for the brief's `[convoy-phase: <tag>]` and for mutating tool use, skips read-only subagents, runs `gate_service.run_gate` in the event's `cwd`, and on a blocking red exits 2 with the repair brief (the subagent's reason not to stop) unless `stop_hook_active` says this is already the retry, when the residual red is recorded and the subagent may stop. `PostToolUse` on `Agent|Task` is the messenger for synchronous dispatch: it reuses the judge's verdict for the same `agentId` from `.convoy/hook.log` (within an hour; else it runs the gate) and on a residual red exits 2 with the brief for the orchestrator; an `async_launched` dispatch is recorded, not gated. Discovery: `$CLAUDE_PROJECT_DIR` then the event's `cwd`. Exit 0 silent (no spec, untrusted, non-dispatch tool, read-only subagent, green); exit 2 with a one-line reason when the gate could not run. One JSON line per firing in `.convoy/hook.log`. Executes a project's checks only where `gate_service.trust_status` is `trusted`: the project root is on the per-machine trust list with the spec's hash (`CONVOY_HOME/hook-trust.toml`, written by `convoy gate --trust`), or in `CONVOY_TRUSTED_ROOTS` from the launching process; an untrusted spec runs nothing and writes nothing, a changed spec is refused loudly, a workspace under a `convoy run` lock is refused. Reads the event as UTF-8 bytes. The judge reads the subagent transcript for the brief, the model and the tool use (anything not on the read-only list is a write); the messenger reuses the judge's record keyed on session and agent. No MCP twin: the caller is Claude Code itself |
...
| `convoy unlock SERIES [--workspace DIR] [--force]` | — | the safe recovery verb: no tool equivalent. Releases a stale run lock and nothing else — no discarded changes, no deleted branches — and closes a killed run's ledger entry the same way `clean` does. This is what a `dead` run's status message names, since `clean`'s tree-wiping steps would delete the branches `--resume` needs. Refuses when `workspace_lock.lock_ownership` says the lock's owner is still alive — the same predicate `convoy status` reads its `dead`/`running` split from — naming the pid; `--force` overrides for a reused pid or a distrusted check |
