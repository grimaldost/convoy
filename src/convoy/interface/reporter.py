...
class StderrReporter:
    ...
    def budget_nearing(self, pr_id: str, role: str, spend_usd: float, cap_usd: float) -> None:
        """One line when a spawn came close to its ceiling, right after its own line.

        Narrated because the operator watching a long run IS the monitor this signal is
        for: the recorded workaround for a busted cap was raising it for the whole wave
        after the fact, which is the expensive version of what one line here enables.
        """
        label = _ROLE_LABEL.get(role, role)
        percent = spend_usd / cap_usd * 100 if cap_usd else 0.0
        self._line(
            f'[{pr_id}] {label:<6} near cap  ${spend_usd:.4f} of ${cap_usd:.2f} ({percent:.0f}%)'
        )
